...
export default function Card({ item, category }: { item: Item, category: string }) {
    return <a className="relative group hover:shadow-xl transition duration-150 ease-in-out" style={!item.tag.includes(category) ? { display: 'none' } : {}} href={`/${item.id}`} data-aos="fade-down" data-aos-anchor="[data-aos-id-inpspiration]">
        <Image className="w-full aspect-square object-cover" src={item.src} width="352" height="352" alt="Inspiration 01" />
        {/* Content on hover */}
